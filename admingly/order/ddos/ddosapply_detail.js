    ...
    fs.readFile('./static/txt/admin_center_nav.txt', function(err, data) {
        if (err) {
            ...
            router.get('/', (req, res) => {
                ...
                db.query(`SELECT * FROM view_refund_ddosapply where refund_id='${refund_id}'`, (err, data) => {
                    if (err) {
                        console.error(err);
                        res.redirect('/error');
                        ...
                        if (common.debugData(data)) {
                            ...
                            db.query(`SELECT * FROM ddos where ddos_id='${data[0].refund_ddosid}'`, (err, data1) => {
                                if (err) {
                                    console.error(err);
                                    res.redirect('/error');//res.status(500).send('database error').end();
                                } else {

                                    res.render('admingly/web/admin_center.ejs', { data, data1, nav, menu, content: "ddosapply_view", menuClick: 1, itemClick: 1 });
                                }
                            });
                        } else {
                            res.redirect('/error');
            ...
            router.post('/refund', urlencodedParser, function(req, res) {
                ...
                var status;
                var message;     
                db.query(`CALL proc_refund_ddos ('${act}','${order_id}','${user_id}',${price},'${clerk}','${moment().format("YYYY-MM-DD HH:mm:ss")}','ddos高防ip退款',@status);`, function(err, rows, fields) {
                        if (err) {
                            console.error(err);
                            res.redirect('/error');//res.status(500).send('database error').end();
                        } else {
