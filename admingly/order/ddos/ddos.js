...
module.exports = function() {
    ...
    fs.readFile('./static/txt/admin_center_nav.txt', function(err, data) {
        if (err) {
            ...
            router.get('/', (req, res) => {
                res.render('admingly/web/admin_center.ejs', { nav, menu, content: "order_ddos", menuClick: 0, itemClick: 1 })
            });
        }
    });
        ...
        db.query(`SELECT permission_role_id FROM admin WHERE admin_id = '${req.session['admin_id']}'`, (err, data1) => {
            if (err) {
                ...
                if (data1[0].permission_role_id == "1004") {
                    ...
                    querystring = `SELECT order_id,order_ip,order_duration,order_totalprice,order_begin,\
                    order_deploy,order_end,order_ddosname,order_status,order_paystatus,order_buyername,\
                    order_userqq,order_clerkname FROM view_order_ddos WHERE order_clerkid = '${req.session['admin_id']}'\
                    and `;
                } else {
                    querystring = `SELECT order_id,order_ip,order_duration,order_totalprice,order_begin,\
                    order_deploy,order_end,order_ddosname,order_status,order_paystatus,order_buyername,\
                    order_userqq,order_clerkname FROM view_order_ddos where \
                    `;}
                if (req.query.ip) {
           ...
           db.query(`SELECT COUNT(1) as count FROM view_order_ddos where ${isclerk} order_begin BETWEEN '${left}' AND '${right}'`, (err, data3) => {
            if (err) {
                console.error(err);
                res.redirect('/error');
              ...
              count=data3[0].count;
                ...
                for (var i = 0; i < data.length; i++) {
                            ...
                            data[i].order_duration=data[i].order_duration+"个月";
