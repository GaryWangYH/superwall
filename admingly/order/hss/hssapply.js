...
module.exports = function() {
    ...
    fs.readFile('./static/txt/admin_center_nav.txt', function(err, data) {
        if (err) {
            ...
            router.get('/', (req, res) => {
                res.render('admingly/web/admin_center.ejs', { nav, menu, content: "hssapply", menuClick: 1, itemClick: 0 })
            });
            router.get('/table', function(req, res) {
                ...
                querystring = `SELECT refund_id,refund_orderid,refund_productname,refund_price,\
                    refund_applyer,refund_time,refund_reason,refund_clerkname,refund_status\
                    FROM view_refund_hssapply WHERE refund_clerkid = '${req.session['admin_id']}'\
                    and refund_time BETWEEN '${left}' AND '${right}' order by refund_time desc\
                    limit ${begin},${limitlength}`;
                } else {
                    querystring = `SELECT refund_id,refund_orderid,refund_productname,refund_price,\
                    refund_applyer,refund_time,refund_reason,refund_clerkname,refund_status\
                    FROM view_refund_hssapply WHERE refund_time BETWEEN '${left}' AND '${right}' order by refund_time desc\
                    limit ${begin},${limitlength}`;
                }
                db.query(`SELECT COUNT(1) as count FROM view_refund_hssapply where ${isclerk} refund_time BETWEEN '${left}' AND '${right}'`, (err, data3) => {
                if (err) {
                console.error(err);
                res.redirect('/error');//res.status(500).send('database error').end();
                } else {
