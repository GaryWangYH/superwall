                ...
                db.query(`SELECT permission_role_id,permission_role_name \
                        FROM  permission_role `,(err,data1)=>{
                            if (err) {
                                ...
                                res.render('admingly/web/admin_center.ejs', {permission_role:data1, nav, menu, content: "log_admin", menuClick: 1, itemClick: 0 })
                            }
                        })
            });
            // router.get('/', (req, res) => {
                ...
                db.query(`SELECT * FROM view_log_admin WHERE log_id = '${req.query.id}'`,(err,data)=>{
                    if(err){
                        ...
                        if (common.debugData(data)) {
                            ...
                            res.render('admingly/web/admin_center.ejs', {data:data[0], nav, menu, content: "log_detail", menuClick: 1, itemClick: 0 })
                        } else {
                            res.redirect('/error');
                        }
                    }
