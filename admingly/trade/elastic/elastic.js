...
module.exports = function() {
    ...
    fs.readFile('./static/txt/admin_center_nav.txt', function(err, data) {
        if (err) {
            ...
            router.get('/', (req, res) => {
                res.render('admingly/web/admin_center.ejs', { nav, menu, content: "trade_elastic", menuClick: 1, itemClick: 2 });
            });
           ...
           if (err) {
                        ...
                        if (data1[0].permission_role_id == "1004") {
                            ...
                            querystring = `SELECT trade_id,order_time,order_totalprice,\
                        order_status,order_buyername,order_ipsum,order_begin,order_userqq,\
                        order_clerkname,pay_type,pay_type_status.name AS pay_type_name \
                        FROM view_trade_elastic LEFT JOIN pay_type_status \
                        ON view_trade_elastic.pay_type = pay_type_status.status \
                        WHERE order_clerkid = '${req.session['admin_id']}'\
                        and order_begin BETWEEN '${left}' AND '${right}' order by order_begin desc\
                        limit ${begin},${limitlength}`;
                        } else {
                            querystring = `SELECT trade_id,order_time,order_totalprice,\
                        order_status,order_buyername,order_ipsum,order_begin,\
                        order_userqq,order_clerkname, pay_type,pay_type_status.name AS pay_type_name \
                        FROM view_trade_elastic LEFT JOIN pay_type_status \
                        ON view_trade_elastic.pay_type = pay_type_status.status \
                        where order_begin BETWEEN '${left}' AND '${right}' order by order_begin desc\
                        limit ${begin},${limitlength}`;
                        }
                        db.query(`SELECT COUNT(1) as count FROM view_trade_elastic where ${isclerk} order_begin BETWEEN '${left}' AND '${right}'`, (err, data3) => {
                            if (err) {
                                console.error(err);
                                res.redirect('/error');//res.status(500).send('database error').end();
                            } else {
                                ...
                                db.query(querystring, (err, data) => {
                                    if (err) {
                                        ...
                                        for (var i = 0; i < data.length; i++) {
                                            ...
                                            data[i].order_begin = moment(data[i].order_begin).format('YYYY-MM-DD HH:mm');
                                        }
                                        start = parseInt((page - 1) % 10 * limit);
                                        end = parseInt(start) + parseInt(limit);
                                        if (page * limit > count) end = start + count % limit;
