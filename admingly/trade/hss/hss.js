...
module.exports = function() {
    ...
    fs.readFile('./static/txt/admin_center_nav.txt', function(err, data) {
        if (err) {
            ...
            router.get('/', (req, res) => {
                res.render('admingly/web/admin_center.ejs', { nav, menu, content: "trade_hss", menuClick: 1, itemClick: 0 });
            });
           ...
           if (err) {
                        ...
                        order_productname,order_status,order_paystatus,order_buyername,order_ipsum,\
                        order_userqq,order_clerkname,pay_type,pay_type_status.name AS pay_type_name \
                        FROM view_trade_hss LEFT JOIN pay_type_status \
                        ON view_trade_hss.pay_type = pay_type_status.status \
                        ...
                        db.query(`SELECT COUNT(1) as count FROM view_trade_hss where ${isclerk} order_begin BETWEEN '${left}' AND '${right}'`, (err, data3) => {
                            if (err) {
                                console.error(err);
                                res.redirect('/error');//res.status(500).send('database error').end();
                            } else {
