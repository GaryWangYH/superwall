...
module.exports = function() {
    ...
    fs.readFile('./static/txt/admin_center_nav.txt', function(err, data) {
        if (err) {
            ...
            router.get('/', (req, res) => {
                res.render('admingly/web/admin_center.ejs', { nav, menu, content: "industryNews", menuClick: 1, itemClick: 1 });
            });
        }
    });

    router.use("/new", require('./news_view.js')(1, "new"));

    router.use("/edit", require('./news_view.js')(1, "edit"));

    router.post("/editSubmit", urlencodedParser, (req, res) => {
        console.log("industryNews");
        ...
        if (operation == "new") {
            db.query(`INSERT INTO data_news VALUES \
                ...
                if (err) {
                    ...
                    //** 添加日志 **
                    common.log_admin(req.session['admin_id'],"新增新闻",`新增一条新闻`);

                    res.status(200).send({ msg: "新建成功", type: 1 }).end();
                }
            });
            ...
            db.query(`UPDATE data_news SET news_type = '${type}', news_inHomePage = '${inHomePage}',\
                ...
                (err, data) => {
                    if (err) {
                        ...
                        //** 添加日志 **
                        common.log_admin(req.session['admin_id'],"修改新闻",`修改一条新闻`);

                        res.status(200).send({ msg: "修改成功", type: 1 }).end();
                    }
                });
        }
    });
        ...
        db.query(`DELETE FROM data_news WHERE news_id = '${news_id}'`,(err,data)=>{
            if (err) {
                ...
                res.redirect('/error');//res.status(500).send('database error').end();
                    ...
                    //** 添加日志 **
                    common.log_admin(req.session['admin_id'],"删除新闻",`删除一条新闻`);
    ...
    router.get('/data', (req, res) => {
        ...
        var end = page * limit;
        db.query(`SELECT data_news.*,admin.admin_name FROM data_news,admin WHERE news_type = 1 \
