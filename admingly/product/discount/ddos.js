...
module.exports = function() {
    ...
    router.get("/",(req,res)=>{
    	db.query(`SELECT * FROM data_ddos_discount ORDER BY value`,(err,data)=>{
    		if(err){
    			console.log(err);
    		}else{
    ...
    router.post("/modify",urlencodedParser,(req,res)=>{
    	...
    	db.query(`UPDATE data_ddos_discount SET value = '${value}',\
    		...
    		if(err){
                ...
                //** 添加日志 **
                common.log_admin(req.session['admin_id'],"修改一条DDoS高防折扣",
    ...
    router.post("/new",urlencodedParser,(req,res)=>{
    	var value = req.body.value;
    	var discount = req.body.discount;
    	db.query(`INSERT INTO data_ddos_discount(id,value,discount) \
    		...
    		if(err){
                ...
                //** 添加日志 **
                common.log_admin(req.session['admin_id'],"新增一条DDoS高防折扣",
    ...
    router.post("/delete",urlencodedParser,(req,res)=>{
    	...
    	var id = req.body.id;
    	var discount = req.body.discount;
    	db.query(`DELETE FROM data_ddos_discount WHERE id = '${id}'`,(err,data)=>{
    		if(err){
    			console.log(err);
    		}else{

                //** 添加日志 **
                common.log_admin(req.session['admin_id'],"删除一条DDoS高防折扣",
